...
async def viewBook(request: Request, bookuid: str, session: AsyncSession = Depends(getSession)):
    book = await bookService.getBookByUid(bookUid=bookuid, userUid='019b91ed-5ca4-7c7b-a31a-8534494d622b', session=session)
    if not book:
    ...
    return template.TemplateResponse(name='viewer.html', context={'request': request, 'url': pdfURL, 'filename': book.filename})
