...
class JobCreator:
    ...
    async def pageRangeSelection(self, total: int) -> Tuple[Literal[0], int] | List[Tuple[int, int]]:
        if total < Config.RANGE:
            return (0, total - 1)
        ranges: List[Tuple[int, int]] = [(i, min(i + Config.RANGE - 1, total - 1)) for i in range(0, total, Config.RANGE)]
        return ranges
    
    async def createDupeKey(self, start: int, end: int, book_id: str, user_id: str, jobtype: str):
            ...
            if not book.duplicate:
                ...
                if isinstance(rangePages, list):
                    for i, (start, end) in enumerate(rangePages):
                        jobtype = JobTypeEnum.bootstrap if i == 0 else JobTypeEnum.background
                        dupeKey = await self.createDupeKey(start, end, str(book.uid), str(book.user_uid), jobtype)
                        jobsData.append({
                            'book_uid': str(book.uid),
                            'user_uid': str(book.user_uid),
                            'page_total': end - start + 1,
                            'job_type': jobtype,
                            'priority': JobPriorityEnum.high if jobtype == JobTypeEnum.bootstrap else JobPriorityEnum.low,
                            'page_start': start,
                            'page_end': end,
                            'dedupekey': dupeKey
                        })
                    ...
                    dupeKey = await self.createDupeKey(start, end, str(book.uid), str(book.user_uid), JobTypeEnum.bootstrap)
                    jobsData.append({
                        'book_uid': str(book.uid),
                        'user_uid': str(book.user_uid),
                        'page_total': end - start + 1,
